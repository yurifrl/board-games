...
const dateLabel = (timestamp: number): string => new Intl.DateTimeFormat("en-US", { dateStyle: "medium" }).format(new Date(timestamp));
...
const EmptyProvider: FC<{ provider: Provider; id: string }> = ({ provider, id }) => {
  ...
  return <><SourceHeader provider={provider} id={id} facts={facts} /><div class="provider-empty"><strong>Provider data has not been fetched yet.</strong><span>Configure the provider API access and run the sync.</span></div></>;
};
...
const BggPane: FC<{ id: string; snapshot?: ProviderSnapshot }> = ({ id, snapshot }) => {
  ...
  return <>
    ...
    <ImageGrid title="Images and editions" images={images} />
    ...
    {related.length ? <section class="provider-section"><h3>Related games</h3><div class="provider-related">{related.map((item) => <a href={`https://boardgamegeek.com/boardgame/${item.id}`} target="_blank" rel="noopener"><strong>{item.name}</strong><small>{item.inbound === "true" ? "Expansion of this game" : "Expansion"}</small></a>)}</div></section> : null}
...
const LudopediaPane: FC<{ id: string; snapshot?: ProviderSnapshot }> = ({ id, snapshot }) => {
  ...
  return <>
    ...
    <FactStrip facts={facts} />
    {description ? <section class="provider-section provider-about"><h3>About</h3><p>{description}</p></section> : null}
    <section class="provider-columns"><Chips title="Mechanics" values={facts.mechanics} /><Chips title="Categories" values={facts.categories} /><Chips title="Designers" values={facts.designers} /><Chips title="Publishers" values={facts.publishers} /></section>
    <VideoGrid videos={videos} />
